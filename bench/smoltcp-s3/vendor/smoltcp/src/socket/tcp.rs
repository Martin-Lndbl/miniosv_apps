...
/// set RTO <- 1 second,
///
/// LOCAL PATCH (was 1000). This applies to the SYN, which by definition has no
/// RTT sample yet, so every lost SYN cost a full second. Measured in-region S3
/// RTT is under 1 ms, making 1000 ms roughly 3000x the round trip; 50 ms is
/// still ~150x and bounds a lost SYN at 50 ms instead of 1000.
const RTTE_INITIAL_RTO: u32 = 50;
...
//
// LOCAL PATCH (was 1000). The floor overrode every measured RTT, so even after
// sampling a sub-millisecond path the stack still waited a second to recover a
// loss. RFC 6298's 1 s is written for the open internet; this stack only ever
// talks to an in-region S3 endpoint. 20 ms is ~60x the measured RTT, which
// leaves ample margin against jitter while cutting recovery 50x.
const RTTE_MIN_RTO: u32 = 20;
